...
/*
Routines to make outputting data easy at a high level
*/
...
void HandleOutput_log(const char* log_str){
  csc_write_data(CSC_CMD_WRITE_PI_LOG, (byte*)log_str, strlen(log_str) );
}

void HandleOutput_error(const char* error_str){
  csc_write_data(CSC_CMD_WRITE_PI_ERROR, (byte*)error_str, strlen(error_str) );
}
